...
class VISTAParam():
    ...
    def __init__(
        ...
        sig: Optional[float] = None,  # Std of the Gaussian envelope for sampling density (default: PE/6)
        w: Optional[float]   = None,  # Scaling of time dimension; frames are "w" units apart (default: max((PE/n)/10+0.25, 1))
        ...
        g: Optional[int]     = None,  # Every gth iteration is relocated on a Cartesian grid (default: nIter//6)
        uni: Optional[int]   = None,  # At uni iteration, reset to equivalent uniform sampling (default: round(nIter/2))
        ...
        fl: Optional[int]    = None,  # Start checking fully sampledness at fl^th iteration (default: round(5/6*nIter))
    ):
        self._PE    = PE
        self._FR    = FR
        self._n     = n
...
def VISTA(param: VISTAParam, rng: np.random.RandomState = None, max_FR: int = 32, cache: bool = True,
          cache_ignore_seed: bool = True) -> np.ndarray:
    """
...
def _VISTA_impl(param: VISTAParam, rng: np.random.RandomState = None) -> np.ndarray:

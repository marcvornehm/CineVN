...
def psnr(gt: torch.Tensor, pred: torch.Tensor, maxval: torch.Tensor | None = None) -> torch.Tensor:
...
def ssim(gt: torch.Tensor, pred: torch.Tensor, maxval: torch.Tensor | None = None) -> torch.Tensor:
    ...
    pred = pred[None].to(torch.float64)
    gt = gt[None].to(torch.float64)
...
def ssim_xt(
    ...
    for i in range(n_profiles // 2):
        ...
        ssim_vals.append(ssim(gt_profile1[None], pred_profile1[None], maxval=maxval))
        ssim_vals.append(ssim(gt_profile2[None], pred_profile2[None], maxval=maxval))
...
def hfen(gt: torch.Tensor, pred: torch.Tensor, sigma: float = 1.5) -> torch.Tensor:
    ...
    if gt.is_cuda and cp is not None:
        ...
        filtered = gaussian_laplace_cpu(gt.cpu().numpy() - pred.cpu().numpy(), sigma=sigma)
    return torch.linalg.norm(torch.as_tensor(filtered, device=gt.device))


class _VolumeAverageMetric(Metric, ABC):
    """
...
class PSNRMetric(_VolumeAverageMetric):
    def update(
            ...
            maxvals: Sequence[torch.Tensor | None],
    ):
...
class SSIMXTMetric(_VolumeAverageMetric):
    ...
    def update(
            ...
            targets: torch.Tensor,
            predictions: torch.Tensor,
            centers: Sequence[torch.Tensor | None],
            maxvals: Sequence[torch.Tensor | None],
    ):

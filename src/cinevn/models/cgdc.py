...
def zdot_reduce_sum(input_x: torch.Tensor, input_y: torch.Tensor) -> torch.Tensor:
    # take only real part
...
def EhE_Op(x: torch.Tensor, csm: torch.Tensor, mask: torch.Tensor, mu: torch.Tensor, fft_ndim: int) -> torch.Tensor:
    """
...
def rhs_Op(y: torch.Tensor, z: torch.Tensor, csm: torch.Tensor, mu: torch.Tensor, fft_ndim: int) -> torch.Tensor:
    """
    Computes E^H*y + mu*z
    """
...
class CGDCAutogradFun(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output: torch.Tensor) -> tuple[None, torch.Tensor, None, None, None, None, None, None]:

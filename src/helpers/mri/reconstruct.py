...
def pics(
    ...
    if load_if_exists:
        assert fname is not None, '`fname` must be given if `load_if_exists` is True'
        try:
            recon = pics_impl.read(fname, layout='bart')  # [x, y, frame, slice]
            return xp.asarray(recon)
        except FileNotFoundError:
            pass

    recon = pics_impl.bart_pics(  # [x, y, frame, slice]
        k_data, regularizer, coil_sens=coil_sens, coil_sens_path=coil_sens_path, coil_sens_nmaps=coil_sens_nmaps,
        fname=fname, phase_crop=phase_crop, use_gpu=use_gpu, suppress_stdouterr=True, write_h5=(fname is not None),
        write_gif=False,
    )

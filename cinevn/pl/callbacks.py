...
from ..data.transforms import apply_affine_to_annotations, apply_affine_to_image
...
class ValImageLogger(pl.Callback):
            ...
            for i, sample_index in enumerate(sample_indices):
                if sample_index in self.val_log_indices:
                    ...
                    if annotations.isnan().any():
                        ...
                        annotations = apply_affine_to_annotations(annotations, target.shape[-2:], *affine.int())
                    output = apply_affine_to_image(output, *affine.int())  # type: torch.Tensor  # type: ignore
                    target = apply_affine_to_image(target, *affine.int())  # type: torch.Tensor  # type: ignore
    ...
    def log_images(self, trainer: pl.Trainer, key: str, target: torch.Tensor, output: torch.Tensor, error: torch.Tensor,
                   annotations: torch.Tensor | np.ndarray | None = None, t_res: float = 0):
        assert target.ndim == output.ndim == error.ndim
        match target.ndim:
            case 2:  # static data
                if trainer.current_epoch == 0:
                    self.log_image(trainer, f'{key}/target', target)
                self.log_image(trainer, f'{key}/reconstruction', output)
                self.log_image(trainer, f'{key}/error', error)
            ...
            case _:
                raise RuntimeError(f'Unexpected number of dimensions {target.ndim}')

    @staticmethod
    def log_image(trainer: pl.Trainer, name: str, image: torch.Tensor, global_step: Optional[int] = None):
        """
    ...
    @staticmethod
    def log_video(trainer: pl.Trainer, name: str, video: torch.Tensor, global_step: Optional[int] = None,
                  fps: float = 8):
        """
    ...
    @staticmethod
    def create_profiles(cine, center, stretch_factor: int = 5):
        xt_profile = cine[:, int(center[1]), :]
        yt_profile = cine[:, :, int(center[0])]
    ...
    @staticmethod
    def draw_crosshairs(cine, center):
...
class SaveReconstructions(pl.Callback):
    ...
    @property
    def state_key(self) -> str:
        return self._generate_state_key(save_img=self.save_img)
    ...
    def save_reconstructions(self, out_dir: Path, reconstructions: Dict[str, np.ndarray], affines: Dict[str, np.ndarray],
                             headers: Dict[str, ismrmrd.xsd.ismrmrdHeader], positions: Dict[str, np.ndarray],
        ...
        for fname in reconstructions:
            ...
            # save as png/gif files
            if self.save_img:
                img_dir = out_dir / 'img'
                img_dir.mkdir(exist_ok=True)

                # normalize orientation
                recon = apply_affine_to_image(recon, *affine.astype(int))
                ...
                for s in range(num_slices):
                    fname_slice = f'{Path(fname).stem}_slice{s:02d}'
                    recon_slice = recon[s]

                    # save magnitude image
                    conversion.array2gif(
                        np.abs(recon_slice), img_dir / f'{fname_slice}_mag', clip=True, tres=tres
                    )

                    # save phase image
                    conversion.array2gif(
                        np.angle(recon_slice), img_dir / f'{fname_slice}_phi', clip=False, vmin=-np.pi, vmax=np.pi,
                        tres=tres
                    )
...
class AccelerationScheduler(pl.Callback):
    def __init__(self, acc_target: int, acc_start: int = 2, acc_incr: int = 1,
                 step_size: int = 1, target_epoch: Optional[int] = None):
        """
        Args:

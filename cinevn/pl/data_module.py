...
from ..data import ClusteredBatchSampler, SliceDataset, VolumeSampler
...
class FastMriDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        Args:
            ...
            train_source: Name of data set source for training (overwrites data_source for this dataset)
            val_source: Name of data set source for validation (overwrites data_source for this dataset)
            test_source: Name of data set source for testing (overwrites data_source for this dataset)
            ...
            coilsens_path: Path to coil sensitivity maps.
    ...
    def _create_data_loader(
        ...
        coilsens_path = self.coilsens_path / source if self.coilsens_path is not None else None
    ...
    def prepare_data(self):
        ...
        if self.use_dataset_cache_file:
            fn = functools.partial(SliceDataset, use_dataset_cache=self.use_dataset_cache_file,
                                   dataset_cache_file=self.dataset_cache_file)
            fn(root=self.data_path / f'{self.train_source}_train',
               transform=self.train_transform,
               sample_rate=self.sample_rate,
               volume_sample_rate=self.volume_sample_rate,
               coilsens_path=self.coilsens_path / f'{self.train_source}' if self.coilsens_path is not None else None)
            fn(root=self.data_path / f'{self.val_source}_val',
               transform=self.val_transform,
               sample_rate=1.0,
               volume_sample_rate=None,
               coilsens_path=self.coilsens_path / f'{self.val_source}' if self.coilsens_path is not None else None)
            fn(root=self.data_path / f'{self.test_source}_{self.test_split}',
               transform=self.test_transform,
               sample_rate=1.0,
               volume_sample_rate=None,
               coilsens_path=self.coilsens_path / f'{self.test_source}' if self.coilsens_path is not None else None)
    ...
    def val_dataloader(self):
        ...
        return self._create_data_loader(self.val_transform, data_partition='val', sample_rate=1.0)

    def test_dataloader(self):
        if self.test_transform is None:
            raise ValueError('test_transform must be set.')
        return self._create_data_loader(self.test_transform, data_partition=self.test_split, sample_rate=1.0)
